...
<body>
	...
	<div class="container">
				...
				<div class="row ">
					<div class="input-field col s12 l3 m4">
						<input placeholder="start" name="start"
							value="<%=null == request.getParameter("start") ? "" : request.getParameter("start")%>"
							type="text">
					</div>
					<div class="input-field col s12 l3 m4">
						<input placeholder="end" name="end"
							value="<%=null == request.getParameter("end") ? "" : request.getParameter("end")%>"
							type="text">
					</div>
					<div class="input-field col s12 l3 m4">
						<select id="sort" name="sort">
							...
							<option value="1" class="cyan-text"
								<%=null != request.getAttribute("sort")
					|| (null != request.getParameter("sort") && 1 == Integer.valueOf(request.getParameter("sort")))
							? "selected" : ""%>>time</option>
							<option value="2"
								<%=null != request.getParameter("sort") && 2 == Integer.valueOf(request.getParameter("sort"))
		...
		%>
		<div>
			<h4 class="thin center-align white-text">
				No.<span
					class=<%=cnt == 0 ? "red-text text-accent-2"
							: cnt == 1 ? "orange-text text-accent-2"
									: cnt == 2 ? "teal-text text-accent-2" : "white-text"%>><%=++cnt%></span>
			</h4>
			<hr class="" width=180px color=#ffffff size="1" align=center noshade>
			<div class=" center-align white-text">
				<span>Time:<%=path.time%>min
				</span> <span>Transfer:<%=path.transfer - 1%></span>
			</div>
			<div class="border-side-white no-margin cyan">
				<div class=" path-card-margin">
					...
					<p class="center-align no-margin" style="line-height: 318%">
						...
						<!-- 站点 -->
						<a
							class='path-station-change path-station-rnd  disabled  cyan-text  z-depth-1 '>
							<%=line.get(i)%></a>
						<!-- 箭头和信息 -->
						<a class=" btn-flat disabled btn-rnd white-text"
							style="font-size: 15px; margin: 0px; padding: 0px"> --<%=line.get(i + 1)%>(<%=busTime.get(j)%>min)
							-->
						</a>
						<%
							}
